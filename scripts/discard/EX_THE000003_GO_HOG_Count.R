...
#plots the number of HOGs with particular GO Term, which is derived from the sets
...
plot_GO_count_graph <- function(Terms)
{
  ...
  else
  {
    ...
    Terms_order <- Terms[order(Terms$HOGs_with_GO_COUNT_unique, decreasing=F),]
    ...
    ggplot(Terms_order) + 
      geom_bar(aes(x = HOGs_with_GO_COUNT_unique , y = fct_inorder(GOTERM)), stat = "identity") +
      xlab("HOGs containting GO-Term Counts") +
      ylab("GO-Terms") +
...
{
  count_table$HOGs_with_GO_COUNT_unique <- rep(0, length(count_table$TermID))
  ...
  for(GOTERM_parent in count_table$TermID)
  {
    ...
    HOGlist <- unlist(str_split(count_data[count_data$GOID == GOTERM_parent,]$HOGs_with_GO, pattern = " "))
    ...
    {
      HOGlist <- append(HOGlist, unlist(str_split(count_data[count_data$GOID %in% GOTERM_children,]$HOGs_with_GO, pattern = " ")))
      # mark as clustered
      count_table[count_table$TermID == GOTERM_parent,]$Clustered = TRUE
    }
    count_table[count_table$TermID == GOTERM_parent,]$HOGs_with_GO_COUNT_unique <- length(unique(HOGlist))
  }
...
#non reduced plots. Not very usefull
...
plot_GO_count(MSZ_expanded_deg_any_exp_species_reduced05_GO_count_table, "Overall Reduced (0.5) HOG with GO-Term Count\nC4 vs C3, Expanded, DEG in any expanded species", "MsSbZmvsTaOs_Expanded_DEGinAnyExpandedSpecies\\GOCount_Plots\\MsSbZmvsTaOs_Expanded_DEGinAnyExpandedSpecies_reduced05_GO_HOG_count.pdf", width = 20, height = 40, rel_h = c(1, 2, 1))
...
plot_GO_count(AllC4Exp_NoDEGC4_AllC3Up_reduced05_GO_count_table, "Overall Reduced (0.5) HOG with GO-Term Count\nAll C4 expanded, No C4 DEG, All C3 upregulated", "AllC4Exp_NoDEGC4_AllC3Up\\GOCount_Plots\\AllC4Exp_NoDEGC4_AllC3Up_reduced05_GO_HOG_count.pdf", width = 20, height = 40)
...
plot_GO_count(AllC4Exp_DEGAnyC4_NoDEGC3_reduced05_GO_count_table, "Overall Reduced (0.5) HOG with GO-Term Count\nAll C4 Expanded, Any C4 DEG, No DEG C3", "AllC4Exp_DEGAnyC4_NoDEGC3\\GOCount_Plots\\AllC4Exp_DEGAnyC4_NoDEGC3_reduced05_GO_HOG_count.pdf", width = 20, height = 40)
...
plot_GO_count(ZmExp_OnlyAnyZmDEG_NoDEGC3_reduced05_GO_count_table, "Overall Reduced (0.5) HOG with GO-Term Count\nZm expanded, Only Any Zm DEG, No C3 DEG", "ZmExp_OnlyAnyZmDEG_NoDEGC3\\GOCount_Plots\\ZmExp_OnlyAnyZmDEG_NoDEGC3_reduced05_GO_HOG_count.pdf", width = 20, height = 40)
...
plot_GO_count(ZmExp_AllZmDEG_NoDEGC3_reduced05_GO_count_table, "Overall Reduced (0.5) HOG with GO-Term Count\nZm expanded, All Zm DEG, No C3 DEG", "ZmExp_AllZmDEG_NoDEGC3\\GOCount_Plots\\ZmExp_AllZmDEG_NoDEGC3_reduced05_GO_HOG_count.pdf", width = 20, height = 40)
